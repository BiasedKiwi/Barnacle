...
class Moderation(commands.Cog):
    ...
    @app_commands.describe(member="The member to kick.", reason="Why do you want to kick this user?")
    async def kick(self, interaction: discord.Interaction, member: discord.Member, *, reason: str = None):
        ...
        try:
            ...
            embed = discord.Embed(title="Something went wrong!", description=f"Something went wrong while trying to kick {member}, make sure they are not a bot.", color=color.gold())
            embed.add_field(name="Debug info", value=f"```{e}```")
            await interaction.response.send_message(embed=embed)
            return
        await interaction.response.send_message(embed=discord.Embed(title="Done!", description=f"{member.mention} has been kicked.", color=color.gold()))
    ...
    @app_commands.describe(member="The member to ban.", reason="Why do you want to ban this user?")
    async def ban(self, interaction: discord.Interaction, member: discord.Member, *, reason: str = None):
        """Ban a user from a server."""
        if reason is None:
            reason = "No reason provided."
        await member.ban(reason=reason)
        await interaction.response.send_message(embed=discord.Embed(title="Done!", description=f"{member.mention} has been banned.", color=color.gold()))
        
    @app_commands.command(name="unban", description="Unban a user from the server.")
    @app_commands.checks.has_permissions(ban_members=True)
    @app_commands.checks.bot_has_permissions(ban_members=True)
    @app_commands.describe(member_id="The ID of the user you would like to unban from the server.")
    async def unban(self, interaction: discord.Interaction, member_id: int = None):  # TODO: Make this command support using usernames.
        ...
        if member_id is not None:
            try:
                ...
                await interaction.response.send_message(embed=discord.Embed(title="Something went wrong!", description="The user ID you specified does not exist or isn't banned.", color=color.gold()))
                return
            await interaction.response.send_message(embed=discord.Embed(title="Done!", description=f"Successfully unbanned {user.name} with ID {user.id}.", color=color.gold()))
        else:
            await interaction.response.send_message(embed=discord.Embed(title="Something went wrong!", description="You need to specify a user ID to unban.", color=color.gold()))
